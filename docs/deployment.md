...
docker run -d \
  -p 8080:80 \
  -v /root/my-local-conf.yml:/app/src/config/conf.yml \
  --name my-dashboard \
  --restart=always \
...
docker run -d \
  -p 4000:80 \
  -v /volume1/docker/dashy/my-local-conf.yml:/app/src/config/conf.yml \
  --name dashy \
  --restart=always \
  lissy93/dashy:latest
```
...
2. Configuration: Fill in you're settings in `./src/config/conf.yml`
