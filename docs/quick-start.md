...
docker run -d \
  -p 8080:80 \
  -v ~/my-conf.yml:/app/src/config/conf.yml \
  --name my-dashboard \
  --restart=always \
...
Config is written in [YAML Format](https://yaml.org/), and saved in [`/src/config/conf.yml`](https://github.com/Lissy93/dashy/blob/master/public/conf.yml).
...
- You can use a Docker volume to pass a config file from your host system to the container
  - E.g. `-v ./host-system/my-local-conf.yml:/app/src/config/conf.yml`
...
Then edit `./src/config/conf.yml` and rebuild the app with `yarn build`
...
4. You can then edit the config in `./src/config/conf.yml` in your repo, and Netlify will rebuild the app
